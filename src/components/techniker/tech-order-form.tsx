...
type TechOrder = {
  ...
  items: {
    ...
    article: {
      ...
      currentStock: number;
      unit: string;
      serialNumbers: { id: string; serialNo: string; isUsed: boolean }[];
    } | null;
    serialNumbers: { id: string; serialNo: string }[];
  }[];
  mobilfunk: {
    id: string;
    type: string;
...
export function TechOrderForm({ order }: { order: TechOrder }) {
  ...
  function handlePick(item: TechOrder["items"][0]) {
  ...
  function handleUnpick(item: TechOrder["items"][0]) {
    setError(null);
    startTransition(async () => {
  ...
  function handleMfSetup(mf: TechOrder["mobilfunk"][0]) {
    if (!techName.trim()) { setError("Bitte zuerst Technikernamen eingeben."); return; }
    setError(null);
  ...
  function handleMfReset(mf: TechOrder["mobilfunk"][0]) {
    startTransition(async () => {
